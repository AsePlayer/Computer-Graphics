...
// lighting
glm::vec3 lightPos(0.0f, 0.0f, 4.5f);
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lightingShader.setInt("shininess", pow(2, i + 1));
        ...
        model = glm::translate(model, glm::vec3(i * 1.5f, 0.0f, i * 1.5f));
        if (i > 3)
        {
            model = glm::translate(model, glm::vec3(-6.0f, -2.0f, -6.0f));
        }
        model = glm::rotate(model, (float)(-(i%4) * 1.57/3), glm::vec3(0.0f, 1.0f, 0.0f));
        if (i > 3)
        {
